...
void /*FUNCTION*/ rsac2(kname1, yarray, nlen, xarray, max_, nerr, kname_s)
...
int *max_, *nerr;
{
	 ...
	 *    MAX:     Size of YARRAY and XARRAY. [i]
	 *=====================================================================
	 * OUTPUT ARGUMENTS:
	 *    YARRAY:  Contains the dependent variable from the file. [fa]
	 *    NLEN:    Number of data points read. [i]
	 *             Will be less than or equal to MAX.
	 *    XARRAY:  Contains the independent variable from the file. [fa]
	 *    NERR:    Error return flag. Set to 0 if no error occcurred. [i]
	 ...
	 *=====================================================================
	 * SUBROUTINES CALLED:
	 *    SACLIB:  INIHDR, INILHF, ZGTFUN, ZOPEN, ZRABS, ZCLOSE, ZGETC,
	 *             GTOUTM, WRTMSG
	 *=====================================================================
	 * LOCAL VARIABLES:
	...
	*nerr = 0;
	...
	nlcdsk = nlcdsk + *npts;
	zrabs( &nun, xarray, *nlen, &nlcdsk, nerr );
	if( *nerr != 0 )
	    goto L_8888;

        if( lswap ){        /* byteswap all the data points. */
            float *ptr ;
            for( idx = 0, ptr = xarray ; idx < *nlen ; idx++, ptr++ ){
                byteswap( (void*)ptr, 4 ) ;
            }
        }
	...
	*ennd = Xarray[*npts];
	...
	/* - Send any error messages to terminal and close the disk file. */
...
void /*FUNCTION*/ rsac2_ (kname1, yarray, nlen, xarray, max_, nerr, kname_s)
char *kname1;   int kname_s;
float yarray[];
int *nlen;
float xarray[];
int *max_, *nerr;
{
	rsac2 ( kname1 , yarray , nlen , xarray , max_ , nerr , kname_s ) ;
}
