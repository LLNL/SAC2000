...
/*    if (! (fptr = fopen(file,"r")  ) )  { */
      if((fptr = fopen(file,"r")) == (FILE *)NULL)  {
        fprintf(stderr, " errno returned from fopen in evresp 0 %s\n",  strerror(errno)); 
#ifdef LIB_MODE
        fprintf(stderr, "%s failed to open file in evresp 1 %s\n", myLabel, file);
        return NULL;
#else
        error_exit(OPEN_FILE_ERROR,"failed to open file in evresp 2 %s", file);
#endif
       }
   }
