...
int *nerr;
{
	...
	int ixstart, ixstop, iystart, iystop, jfile, jfile_, ncret, 
	...
	for( jfile = 1; jfile <= nfiles; jfile++ ){
		...
		if( lxlimits ){
			cmgem.xdelta = (xmaximum - xminimum)/(float)( nxsize - 
			 1 );
			ixstart = max( 1, (int)( (xstart - xminimum)/cmgem.xdelta ) + 
			 1 );
			ixstop = min( nxsize, (int)( (xstop - xminimum)/cmgem.xdelta ) + 
			 2 );
		...
		if( lylimits ){
			cmgem.ydelta = (ymaximum - yminimum)/(float)( nysize - 
			 1 );
			iystart = max( 1, (int)( (ystart - yminimum)/cmgem.ydelta ) + 
			 1 );
			iystop = min( nysize, (int)( (ystop - yminimum)/cmgem.ydelta ) + 
			 2 );
